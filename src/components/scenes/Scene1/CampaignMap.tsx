...
import { Brain, Leaf, Eye, Swords } from 'lucide-react';
    ...
    <div className="flex flex-col items-center justify-center min-h-screen p-8 grid-bg relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl w-full mb-8">
        {quests.map((quest, i) => (
          ...
          >
            <GlowCard
              glowColor={i === 0 ? 'cyan' : i === 1 ? 'green' : 'magenta'}
              hover={true}
              className={`cursor-pointer h-full ${
                i === 0 ? 'ring-1 ring-cyan-400/50 shadow-[0_0_30px_rgba(0,240,255,0.2)]' : 'opacity-70'
              }`}
              onClick={i === 0 ? onSelectQuest : undefined}
            >
              <div className="flex items-center gap-3 mb-3">
                <div className={`p-2 rounded-lg ${i === 0 ? 'bg-cyan-500/20 text-cyan-400' : i === 1 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-fuchsia-500/20 text-fuchsia-400'}`}>
